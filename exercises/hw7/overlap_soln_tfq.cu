...
int main() {
  ...
  for (int i = 0; i < chunks; i++) { //depth-first launch
    size_t chunk_size = ds / chunks;
    size_t offset = chunk_size * i;
    /*cudaMemcpyAsync(d_x + FIXME, h_x + FIXME, (FIXME) * sizeof(ft), cudaMemcpyHostToDevice, streams[FIXME]);*/
    cudaMemcpyAsync(d_x + offset, h_x + offset, (chunk_size) * sizeof(ft), cudaMemcpyHostToDevice, streams[i % num_streams]);
    /*gaussian_pdf<<<((FIXME) + 255) / 256, 256, 0, streams[FIXME]>>>(d_x + FIXME, d_y + FIXME, 0.0, 1.0, FIXME);*/
    gaussian_pdf<<<((chunk_size) + 255) / 256, 256, 0, streams[i % num_streams]>>>(d_x + offset, d_y + offset, 0.0, 1.0, chunk_size);
    /*cudaMemcpyAsync(h_y + i * (ds / chunks), d_y + i * (ds / chunks), (ds / chunks) * sizeof(ft), cudaMemcpyDeviceToHost, streams[i % num_streams]);*/
    cudaMemcpyAsync(h_y + offset, d_y + offset, (chunk_size) * sizeof(ft), cudaMemcpyDeviceToHost, streams[i % num_streams]);
  }
  cudaDeviceSynchronize();
